...
export function useMemberData() {
  ...
  const fetchProfile = useCallback(async () => {
    try {
      ...
      const mappedProfile: MemberProfile = {
        ...
        membershipExpiresAt:
          apiData.expiryDate || apiData.membershipExpiresAt || apiData.props?.expiryDate,
        ...
        maxMemberCountReached:
          apiData.maxMemberCountReached || apiData.props?.maxMemberCountReached,
      };
  ...
  const updateProfile = useCallback(async (data: Partial<MemberProfile>) => {
    try {
      const response = await apiClient.put<{ data: MemberApiResponse }>(
        API_ENDPOINTS.MEMBERS_ME,
        data
      );
      const apiData = response.data;
      const mappedProfile: MemberProfile = {
  ...
  const fetchPayments = useCallback(async () => {
    try {
      ...
      const response = await apiClient.get<{ data: { payments: Payment[] } }>(
        API_ENDPOINTS.MEMBERS_PAYMENTS
      );
  ...
  const fetchDocuments = useCallback(async () => {
    try {
      ...
      const response = await apiClient.get<{ data: { documents: Document[] } }>(
        API_ENDPOINTS.MEMBERS_DOCUMENTS
      );
  ...
  const daysUntilExpiry = profile?.membershipExpiresAt
    ? Math.ceil(
        (new Date(profile.membershipExpiresAt).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
      )
    : null;
