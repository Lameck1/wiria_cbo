...
interface JobCardRequirementsProps {
  requirements: string[];
}

function JobCardRequirements({ requirements }: JobCardRequirementsProps) {
  if (!requirements || requirements.length === 0) {
    return null;
  }

  const previewRequirements = requirements.slice(0, 3);
  const remainingCount = requirements.length - previewRequirements.length;

  return (
    <div className="flex flex-wrap gap-2 border-t border-gray-100 pt-4">
      <span className="mr-1 text-xs font-medium text-gray-400">Key requirements:</span>
      {previewRequirements.map((request, index) => (
        <span
          key={index}
...
export function JobCard({ job, onClick }: JobCardProps) {
  ...
  return (
    ...
    >
      ...
      <JobCardRequirements requirements={job.requirements ?? []} />
    </motion.div>
  );
}
