...
export function ProgramOverviewCard({ program }: ProgramOverviewCardProps) {
  ...
  return (
    <motion.div
      ...
      className="mb-8 overflow-hidden rounded-xl p-8 text-white shadow-lg md:p-12"
    ...
    >
      <AnimatePresence mode="wait">
        ...
        >
          ...
          >
            <ProgramIcon type={program.icon} className="h-20 w-20" />
          </motion.div>
          <div className="flex-grow">
            ...
            <motion.p
              ...
              className={`${styles.subtitleClass} mb-6 text-lg`}
            >
              {program.subtitle}
            </motion.p>
            <div className="grid gap-4 text-white sm:grid-cols-3">
              {program.highlights.map((highlight, index) => (
                ...
                >
                  <CheckIcon className="mr-2 mt-0.5 h-6 w-6 flex-shrink-0" />
                  <span>{highlight}</span>
                </motion.div>
              ))}
            </div>
