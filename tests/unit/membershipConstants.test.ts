...
describe('Membership Constants', () => {
  describe('MEMBERSHIP_FEES', () => {
    ...
    it('should have GROUP fees lower than INDIVIDUAL fees', () => {
      expect(MEMBERSHIP_FEES.GROUP.registration).toBeLessThan(
        MEMBERSHIP_FEES.INDIVIDUAL.registration
      );
      expect(MEMBERSHIP_FEES.GROUP.subscription).toBeLessThan(
        MEMBERSHIP_FEES.INDIVIDUAL.subscription
      );
    });
  });
